...
<html lang="en">
	...
	<%
		/*--------------This code generates datesheet---------------------*/
		
		String batch=(String)request.getParameter("batch");
		if(batch.equals("select") || batch.equals("M.Tech"))
		{
			response.sendRedirect("DateSheeta.jsp");
		}
		out.print(batch);
		...
		*/
		...
		try {
			...
			pst = con.prepareStatement("SELECT * FROM datesheet_course_detail where batch="+"'"+batch+"'");
			studb = con.prepareStatement("SELECT * FROM courses where batch="+"'"+batch+"'");
			out.print(pst);
			out.print(studb);
			rs = pst.executeQuery();
			...
			for(int x = 0; x < listofSlots.size(); x++)
			{
				...
				for(int z = 0; z < listofSlots.get(x).size(); z++)
				{
					...
					q="select count(courses.Enroll_no) from student_detail,courses where student_detail.Enroll_no=courses.Enroll_no and Course_Code="+
						"'"+listofSlots.get(x).get(z)+"'";
					...
					else if(batch.equals("MBA"))
					{
						q="update datesheet_course_detail set Exam_Day_No="+"'"+day+"'"+" , Slot_No="+"'"+m+"'"
						+" where Course_Code="+"'"+listofSlots.get(x).get(z)+"'";
					}
			...
			response.sendRedirect("DateSheetRedirect.jsp");	
